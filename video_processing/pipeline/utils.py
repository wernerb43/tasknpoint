...
def video2frames_with_crop(
    ...
    """Convert input video to images with optional resizing, frame skipping, and frame range selection."""
    if frame_skip is None:
        frame_skip = 1
    if start_frame is None:
        start_frame = 0
    if end_frame is None:
        end_frame = float('inf')  # Process until end of video
...
def prepare_inputs(
    sample_path, output_folder=None, max_height=None, max_fps=None
):
    ...
    if os.path.exists(seq_folder):
        ...
        if len(glob.glob(f"{img_folder}/*.jpg")) > 0:
            return (
                sorted(glob.glob(f"{img_folder}/*.jpg")),
                seq_folder,
                img_folder,
                fps,
            )
    ...
    if isdir:
        ...
        nframes, _ = video2frames(
            sample_path, img_folder, max_height=max_height
        )
...
def convert_av1_to_h264(video_path, output_dir=None):
    ...
    output_path = os.path.join(
        output_dir,
        os.path.splitext(os.path.basename(video_path))[0] + "_h264.mp4",
    )
    cmd = [
        "/usr/bin/ffmpeg",
        "-y",
...
def load_video_frames(
    video_path, output_folder=None, max_height=None, max_fps=None, start_frame=None, end_frame=None
):
    # check if sample_path is an image_folder
    ...
    assert (
        max_fps is not None and max_fps > 0
    ), f"max_fps is not positive: {max_fps}"
    ...
    if start_frame is None and end_frame is None:
        ...
        nframes, frames = video2frames_with_crop(
            video_path,
            max_height=max_height,
            return_images=True,
            frame_skip=frame_skip,
