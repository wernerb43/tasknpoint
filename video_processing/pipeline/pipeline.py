...
from pipeline.detector.vitpose_estimator import (
    load_vit_model,
    estimate_kp2ds_from_bbox_vitpose,
)
...
from data_config import CHECKPOINTS, SMPLX_NEUTRAL_MODEL, SMPLX_NEUTRAL_ARRAY_MODEL
...
class Pipeline:
    def __init__(self, static_cam=False):
        ...
        self.data_dict = {
            "droid": CHECKPOINTS["droid"],
            "sam": CHECKPOINTS["sam"],
            "sam2": CHECKPOINTS["sam2"],
            "yolo": CHECKPOINTS["yolo"],
            "vitpose": CHECKPOINTS["vitpose"],
        }

        self.smplx = SMPLX(
            SMPLX_NEUTRAL_MODEL,  # Use centralized path
            use_pca=False,
            flat_hand_mean=True,
            num_betas=10,
        )
    ...
    def load_frames(self, input_video, output_folder, read_frames=True, start_frame=None, end_frame=None):
        if read_frames == True:
    ...
    def run_detect_track(
        ...
        if self.cfg.tracker == "bytetrack":
            ...
            tracks, masks = detect_segment_track_sam(
                self.images, # T, H, W, 3
                self.seq_folder, # path to where results will be saved
                self.data_dict, # dictionary w dict_keys(['droid', 'sam', 'sam2', 'yolo', 'vitpose']) to model paths
                debug_masks=False,
                sam2_type="tiny",
                detector_type="detectron2",
                num_max_people=self.cfg.num_max_people, # set to 100 people 
                det_thresh=self.cfg.det_thresh, # set to 0.1
                score_thresh=self.cfg.det_score_thresh, # set to 0.1
                height_thresh=self.cfg.det_height_thresh, # set to 0.3
                bbox_interp=self.cfg.bbox_interp, # True
            )
    ...
    def estimate_2d_keypoints(
        self,
    ):
        model = load_vit_model(
            model_path=CHECKPOINTS["vitpose"]\
        )
                ...
                if self.cfg.focal is None and opt_intr == False:
                    try:
                        if self.cfg.calib_method == "ba":
                            ...
                            cam_int = calibrate_intrinsics(
                                self.cfg.img_folder, masks
                            )
                    except ValueError as e:
                        static_cam = True
                        print(e)
                        print(
                            "Warning: probably there is not much camera motion in the video!!"
                        )
                        cam_int = est_calib(self.images)
    ...
    def create_world4d(self, results=None, total=None, step=1):
        ...
        for i in range(0, total, step):
            ...
            if len(track_id) > 0:
                world4d[i] = {
                    "pose": torch.tensor(np.concatenate(pose))
                    .float()
                    .reshape(len(track_id), -1, 3),
    ...
    def create_first_person_world4d(self, results=None, total=None, step=1):
        if results is None:
            results = self.results
        if total is None:
            total = len(results["camera"]["pred_cam_R"])
        ...
        for i in range(0, total, step):
            ...
            # People
            first_pid = (
                next(iter(results["people"])) if results["people"] else None
            )

            # People (only the first PID)
            if first_pid is not None:
                people = results["people"][first_pid]
                frames = people["frames"]
                in_frame = np.where(frames == i)[0]
    ...
    def results_exist(self, output_folder, start_frame=None, end_frame=None, seg_title=None):
        """Check if results file already exists without loading frames."""
        # Construct the expected seq_folder path (same logic as load_video_frames)
        seq_folder = output_folder
        
        if start_frame is not None and end_frame is not None:
            if seg_title is not None:
                pkl_name = f"results_{seg_title}_{start_frame}_{end_frame}.pkl"
            else:
                pkl_name = f"results_{start_frame}_{end_frame}.pkl"
        else:
            pkl_name = "results.pkl"
        
        return os.path.isfile(f"{seq_folder}/{pkl_name}")
    ...
    def __call__(
        ...
        images, seq_folder = self.load_frames(input_video, output_folder, start_frame=start_frame, end_frame=end_frame)
        ...
        if os.path.isfile(f"{seq_folder}/{pkl_name}"):
            print(f"Loading available results at {seq_folder}/{pkl_name} ...")
            self.results = joblib.load(f"{seq_folder}/{pkl_name}")
            return self.results
        ...
        if not self.results["has_slam"]:
            ...
            spec_calib = run_cam_calib(
                self.images,
                out_folder=seq_folder + "/spec_calib",
                save_res=True,
                stride=stride,
                method="spec",
                first_frame_idx=0,
        ...
        # ### post optimization
        print("5. device: ", next(self.smplx.parameters()).device)
        ...
        if start_frame is not None and end_frame is not None:
            self.results["start_frame"] = start_frame
        ...
        #     # TODO
        #     pdb.set_trace()
        print("saving results to ", f"{seq_folder}/{pkl_name}")
        joblib.dump(self.results, f"{seq_folder}/{pkl_name}")

        print("6. device: ", next(self.smplx.parameters()).device)
        ...
        for k, v in self.results["people"].items():
            out_smpl_f = (
                f"{os.path.abspath(self.cfg.seq_folder)}/subject-{k}.smpl"
            )

            SMPLCodec(
                shape_parameters=v["smplx_world"]["shape"].mean(0),
                body_pose=v["smplx_world"]["pose"][:, : 22 * 3].reshape(
                    -1, 22, 3
                ),
            ...
            per_body_frame_presence.append(
                [int(v["frames"][0]), int(v["frames"][-1]) + 1]
            )

        print("7. device: ", next(self.smplx.parameters()).device)
        export_scene_with_camera(
            ...
            frame_rate=float(self.cfg.fps),
            smplx_path=SMPLX_NEUTRAL_ARRAY_MODEL,
        )

        print("8. device: ", next(self.smplx.parameters()).device)
        print("Usage:")
        print(
            f'\tYou can drag and drop the "world4d.mcs" file to https://me.meshcapade.com/editor to view the result'
        )
        print(
            f'\tYou can import the "world4d.glb" file on Blender to view the result'
        )
