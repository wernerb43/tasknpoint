...
from data_config import CHECKPOINTS
...
def softargmax1d(
    ...
    # Should be BxJx2
    keypoints = (normalized_heatmap.reshape(batch_size, -1, dim) * points).sum(
        dim=-1
    )
...
def vfov2soft_idx(angle):
    return angle_to_soft_idx(
        angle, min=np.min(vfov_bins), max=np.max(vfov_bins)
    )


def pitch2soft_idx(angle):
    return angle_to_soft_idx(
        angle, min=np.min(pitch_bins), max=np.max(pitch_bins)
    )
...
def convert_preds_to_angles(
    ...
    if loss_type in ("kl", "ce"):
        ...
        pred_vfov = soft_idx_to_angle(
            get_softargmax(pred_vfov),
            min=np.min(vfov_bins),
            max=np.max(vfov_bins),
        )
        pred_pitch = soft_idx_to_angle(
            get_softargmax(pred_pitch),
            min=np.min(pitch_bins),
            max=np.max(pitch_bins),
        )
        if not legacy:
            pred_roll = soft_idx_to_angle(
                get_softargmax(pred_roll), min=-0.6, max=0.6
            )
        else:
            pred_roll = bins2roll(pred_roll)
    ...
    if return_type == "torch" and isinstance(pred_vfov, np.ndarray):
        return (
            torch.from_numpy(pred_vfov),
            torch.from_numpy(pred_pitch),
            torch.from_numpy(pred_roll),
        )
...
class CameraRegressorNetwork(nn.Module):
    def __init__(
        ...
        if num_fc_layers == 1:
            ...
            self.fc_vfov = self._get_fc_layers(
                num_fc_layers, num_fc_channels, out_channels
            )
            self.fc_pitch = self._get_fc_layers(
                num_fc_layers, num_fc_channels, out_channels
            )
            self.fc_roll = self._get_fc_layers(
                num_fc_layers, num_fc_channels, out_channels
            )
...
def test_model():
    ...
    for b, nl, nc, im_size in list(
        product(backbones, num_fc_layers, num_fc_channels, img_size)
    ):
        print("backbone", b, "n_f_layer", nl, "n_ch", nc, "im_size", im_size)
        inp = torch.rand(1, 3, *im_size)
        model = CameraRegressorNetwork(
            backbone=b, num_fc_layers=nl, num_fc_channels=nc
        )
        out = model(inp)
...
def run_spec_calib(
    ...
    #CKPT = "/data/ilona/code/human_pose/PromptHMR/data/pretrain/camcalib_sa_biased_l2.ckpt"
    CKPT = CHECKPOINTS['camcalib']

    if (
        os.path.exists("/.dockerenv")
        or "AWS_DEFAULT_REGION" in os.environ.keys()
    ):
    ...
    model = load_pretrained_model(
        model, ckpt["state_dict"], remove_lightning=True, strict=True
    )
    ...
    for idx, batch in enumerate(val_dataset):
        ...
        pred_f_pix = orig_img_h / 2.0 / np.tan(pred_vfov / 2.0)
...
@torch.no_grad()
def run_wildcam_calib(
    images=None, out_folder=None, save_res=False, stride=1, first_frame_idx=0
):
    ...
    model = torch.hub.load(
        "ShngJZ/WildCamera", "WildCamera", pretrained=True, verbose=False
    ).to("cuda")
    model.eval()
...
def run_cam_calib(
    ...
    if method == "spec":
        return run_spec_calib(
            images, out_folder, loss_type, save_res, stride, first_frame_idx
        )
    elif method == "wildcam":
        return run_wildcam_calib(
            images, out_folder, save_res, stride, first_frame_idx
        )
