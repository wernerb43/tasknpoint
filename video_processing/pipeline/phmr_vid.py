...
from pipeline.gvhmr.hmr4d.utils.geo.hmr_cam import (
    get_bbx_xys_from_xyxy,
    normalize_kp2d,
)
from prompt_hmr.utils.rotation_conversions import axis_angle_to_matrix

from data_config import CHECKPOINTS, SMPLX_NEUTRAL_MODEL

def load_video_head():
    phmr_vid_cfg = OmegaConf.load(CHECKPOINTS["phmr_vid_config"])
    phmr_vid_ckpt = CHECKPOINTS["phmr_vid_ckpt"]
    vid_head = DemoPL(
        pipeline=phmr_vid_cfg.model.pipeline,
        #smplx_path="/data/ilona/code/human_pose/PromptHMR/data/body_models/smplx/SMPLX_NEUTRAL.npz",
        smplx_path=SMPLX_NEUTRAL_MODEL,
    )
...
class PromptHMR_Video:
    def __init__(
        ...
        super().__init__()
        self.model = load_phmr(CHECKPOINTS["phmr"])
    ...
    def run(self, images, results, mask_prompt=True):
        ...
        cam_intrinsic[0, 2] = (
            img_center[0].item()
            if isinstance(img_center, np.ndarray)
            else img_center[0]
        )
        cam_intrinsic[1, 2] = (
            img_center[1].item()
        ...
        for batch in dataloader:
            ...
            for bid in range(len(batch)):
                ...
                for tid in track_id:
                    ...
                    tracks[tid]["smplx_pose"].append(
                        output[bid]["rotmat"][btid]
                    )
                    tracks[tid]["smplx_transl"].append(
                        output[bid]["transl"][btid]
                    )
                    tracks[tid]["smplx_betas"].append(
                        output[bid]["betas"][btid]
                    )
                    tracks[tid]["prhmr_img_feats"].append(
                        output[bid]["features"][btid]
                    )

        for k, v in tracks.items():
            tracks[k]["smplx_pose"] = torch.stack(
                tracks[k]["smplx_pose"]
            ).float()
            tracks[k]["smplx_transl"] = torch.stack(
                tracks[k]["smplx_transl"]
            ).float()
            tracks[k]["smplx_betas"] = torch.stack(
                tracks[k]["smplx_betas"]
            ).float()
            tracks[k]["prhmr_img_feats"] = torch.stack(
                tracks[k]["prhmr_img_feats"]
            ).float()
        ...
        for idx, k in enumerate(list(tracks.keys())):
            ...
            bbx_xys = get_bbx_xys_from_xyxy(
                torch.from_numpy(tracks[k]["bboxes"])
            ).numpy()

            smoothed = np.array(
                [signal.medfilt(param, 11) for param in bbx_xys.T]
            ).T
            bbx_xys = np.array(
                [gaussian_filter1d(traj, 3) for traj in smoothed.T]
            ).T
            ...
            batch = {
                ...
                "K_fullimg": cam_intrinsic[None, None].repeat_interleave(
                    seqlen, 1
                ),
            ...
            with torch.amp.autocast("cuda"):
                prhmr_vid_output_w_kpts = self.vid_head.pipeline.forward(
                    batch, train=False, postproc=False, static_cam=True
                )

            batch["obs"] = torch.zeros_like(batch["obs"])
            with torch.amp.autocast("cuda"):
                prhmr_vid_output = self.vid_head.pipeline.forward(
                    batch, train=False, postproc=False, static_cam=True
                )

            contact_label = (
                prhmr_vid_output["model_output"]["static_conf_logits"].sigmoid()
                > 0.8
            )
            smplx_pose_aa = torch.cat(
                [
                    prhmr_vid_output["pred_smpl_params_incam"]["global_orient"][
                        0
                    ],
            ...
            rotmat = axis_angle_to_matrix(smplx_pose_aa.reshape(-1, 3)).reshape(
                -1, 25, 3, 3
            )
            ...
            hps_results = {
                ...
                "shape": prhmr_vid_output["pred_smpl_params_incam"]["betas"][0]
                .cpu()
                .float()
                .numpy(),
                "trans": prhmr_vid_output_w_kpts["pred_smpl_params_incam"][
                    "transl"
                ][0]
                .cpu()
                .float()
                ...
                "static_conf_logits": prhmr_vid_output["model_output"][
                    "static_conf_logits"
                ][0]
                .cpu()
                .numpy(),
    ...
    def __getitem__(self, idx):
        ...
        for person_id, track in self.tracks.items():
            if idx in track["frames"]:
                ...
                bbox = np.concatenate(
                    [bbox, np.ones_like(bbox)[..., :1]], axis=-1
                )
