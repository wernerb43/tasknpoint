...
def all_motions_target_velocity_error_exp(
  ...
  source_vel = command.get_source_lin_vel_w()  # (E, S, 3)
  target_vel = command.target_velocity_w  # (E, S, 3) — sampled per episode
  source_dir = source_vel / source_vel.norm(dim=-1, keepdim=True).clamp(min=1e-6)
  target_dir = target_vel / target_vel.norm(dim=-1, keepdim=True).clamp(min=1e-6)
  dot = torch.sum(target_dir * source_dir, dim=-1).clamp(-1.0, 1.0)  # (E, S)
  error = 1.0 - dot
  reward = torch.exp(-(error**2) / std**2)  # (E, S)
