...
def print_probe_results(
  ...
  # Pelvis is body index 0 in body_link_pos_w (root body).
  root_pos_w = torch.from_numpy(log["body_pos_w"][:, 0, :])  # (T, 3)
  root_quat_w = torch.from_numpy(log["body_quat_w"][:, 0, :])  # (T, 4)
...
def run_sim(
  ...
  while not file_saved:
    ...
    if not file_saved:
      ...
      log["site_pos_w"].append(robot.data.site_pos_w[0, :].cpu().numpy().copy())
      log["site_quat_w"].append(robot.data.site_quat_w[0, :].cpu().numpy().copy())
