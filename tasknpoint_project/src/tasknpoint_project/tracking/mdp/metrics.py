...
if TYPE_CHECKING:
  from tasknpoint_project.tracking.mdp.commands import MultiTargetMotionCommand


def compute_mpkpe(command: MultiTargetMotionCommand) -> torch.Tensor:
...
def compute_root_relative_mpkpe(command: MultiTargetMotionCommand) -> torch.Tensor:
...
def compute_joint_velocity_error(command: MultiTargetMotionCommand) -> torch.Tensor:
...
def _get_body_indices(
  ...
  Args:
    command: The multi-target motion command.
