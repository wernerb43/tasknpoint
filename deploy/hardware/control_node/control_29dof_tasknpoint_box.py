##
#
# Control node for the box-pickup policy (hardware).
#
# Goal vector received from hardware_tasknpoint.py (9 floats):
#   [0:3]  right-palm target in pelvis frame  (box_centre + R_pelvis.T @ R_box @ right_grasp_offset)
#   [3:6]  left-palm  target in pelvis frame  (box_centre + R_pelvis.T @ R_box @ left_grasp_offset)
#   [6:9]  otherhand  offset in right-palm local frame  (constant per motion, e.g. [0, 0.25, 0])
#
# Observation layout (163 dims):
#   command (29+29+9=67) + anchor_ori_b (6) + base_ang_vel (3) + qj (29) + dqj (29) + action (29)
#
##
...
class ControlNode(Node):
  ...
  def __init__(self, config_path: str):
    ...
    # goal: right_palm (3) + left_palm (3) + otherhand_offset (3) = 9 floats
    self._goal_dim = 9
  ...
  def init_policy(self):
    ...
    for i, mp in enumerate(self.config["motion_paths"]):
      ...
      # guard against config having fewer contact phases than motions
      cp = contact_phases[i] if i < len(contact_phases) else contact_phases[-1]
      ttc = cp * num_frames * self.ctrl_dt
  ...
  # motion trigger callback — B button (index 4), matching simulation_box.py
  ...
  def build_observation(self):
    ...
    # --- command (67) : motion reference joint_pos + joint_vel + goal_targets (9) ---
    ...
    # concatenate: (29+29+9) + 6 + 3 + 29 + 29 + 29 = 163
    obs = np.concatenate(
