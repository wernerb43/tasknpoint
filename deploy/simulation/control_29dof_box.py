...
class ControlNode(Node):
    ...
    def _control_callback(self):
        # On the very first tick, capture yaw alignment between current robot
        # heading and motion frame 0.
        if self.policy_start_time is None:
            self.policy_start_time = self.sim_time
            motion_anchor_quat_0 = self.motions[0]["body_quat_w"][
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description="Box-pickup control node.")
